...
def main():
    ...
    def loadDataForPlot(sequences):
        ...
        for sequence_num in sequences:
            ...
            interpolation = 1
            RA = helper.getLog(helper.getSumDim(helper.getMagnitude(
                RAD_complex, power_order=2), target_axis=-1), scalar=10, log_10=True)
            RD = helper.getLog(helper.getSumDim(helper.getMagnitude(
                RAD_complex, power_order=2), target_axis=1), scalar=10, log_10=True)
