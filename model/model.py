...
class RADDet(tf.keras.Model):
    ...
    def buildModel(self,):
        ...
        input_tensor = tf.keras.layers.Input(self.input_size)
        ### for convenience ###
        conv = input_tensor
        ...
        self.backbone_stage = tf.keras.Model(input_tensor, features)
        ...
        model = tf.keras.Model(input_tensor, yolo_raw)
        return model
    ...
    def loss(self, pred_raw, pred, gt, raw_boxes):
        box_loss, conf_loss, category_loss = loss_func.lossYolo(
            pred_raw, pred, gt, raw_boxes, self.input_size, self.focal_loss_iou_threshold)
        box_loss *= 1e-1
